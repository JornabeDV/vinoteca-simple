...
export async function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-full">
      ...
      <DesktopSidebar userRole={user.role} businessName={business?.name} businessLogo={business?.logo} />

      {/* Main content area - offset on desktop */}
      <TourClientWrapper userRole={user?.role}>
        <div className="lg:ml-72">
          <Header user={user as any} businessName={business?.name} businessLogo={business?.logo} />
